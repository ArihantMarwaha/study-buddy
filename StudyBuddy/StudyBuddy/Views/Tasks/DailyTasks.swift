...
struct GoalsListView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // (searchable modifier remains on the VStack)

            // Header and capsule in one HStack
            HStack {
                Text("Goals")
                ...
                GlassEffectContainer {
                    HStack(spacing: 0) {
                        ...
                        if isSelectionMode && !selectedGoals.isEmpty {
                            Button(action: deleteSelectedGoals) {
            ...
            if isSelectionMode {
                HStack {
                    Text("\(selectedGoals.count) selected")
                        ...
                        .foregroundColor(.primary)
                    Spacer()
                    // Remove the extra trash button here
                    // (do not show any button in this HStack)
                }
                .padding(.horizontal)
                .padding(.bottom, 8)
        ...
        VStack(spacing: 24) {
            ...
            HStack(spacing: 12) {
                ...
                Button(action: saveGoal) {
                    Text(goal.isDefault ? "Save Changes" : "Save Changes")
                        .font(.headline)
                        .foregroundColor(.white)
                        .frame(maxWidth: goal.isDefault ? .infinity : .infinity)
                        .padding()
