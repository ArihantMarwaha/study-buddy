...
class AudioService: NSObject, ObservableObject {
    ...
    func startRecording() -> URL? {
        ...
        print("[AudioService] Attempting to record to: \(recordingURL.path)")
        ...
        do {
            ...
            print("[AudioService] AVAudioRecorder initialized successfully.")
            audioRecorder?.delegate = self
            audioRecorder?.isMeteringEnabled = true
            audioRecorder?.prepareToRecord()
            let started = audioRecorder?.record() ?? false
            print("[AudioService] audioRecorder?.record() called, started: \(started)")
    ...
    func stopRecording() -> (url: URL, duration: TimeInterval)? {
        ...
        if FileManager.default.fileExists(atPath: url.path) {
            ...
            print("[AudioService] Recording saved at \(url.path), size: \(size) bytes")
        } else {
            print("[AudioService] Recording file not found at \(url.path)")
        }
    ...
    private func startMetering() {
        recordingTimer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { [weak self] _ in
            ...
            // Ensure we're always on the main actor when calling updateMeters()
            Task { @MainActor in
                self.updateMeters()
            }
        }
...
extension AudioService: AVAudioRecorderDelegate {
    nonisolated func audioRecorderDidFinishRecording(_ recorder: AVAudioRecorder, successfully flag: Bool) {
        print("[AudioService] audioRecorderDidFinishRecording called, success: \(flag)")
        if !flag {
...
extension AudioService: AVAudioPlayerDelegate {
    func audioPlayerDidFinishPlaying(_ player: AVAudioPlayer, successfully flag: Bool) {
        ...
        DispatchQueue.main.async {
            ...
            // Optionally, update any UI state or notify observers
