...
struct VoiceRecorderView: View {
    ...
    var body: some View {
        VStack(spacing: 30) {
            ...
            if showingTranscript && !(transcriptionManager.transcription?.isEmpty ?? true) {
        ...
        .onAppear {
            transcriptionManager.requestSpeechPermission()
        }
    ...
    private func toggleRecording() {
        if audioService.isRecording {
            if let result = audioService.stopRecording() {
                ...
                transcriptionManager.transcribeAudio(url: result.url)
            }
        } else {
    ...
    private func saveOrTranscribe() {
        if !showingTranscript {
            ...
            if (transcriptionManager.transcription?.isEmpty ?? true), let url = recordingURL {
                transcriptionManager.transcribeAudio(url: url)
            }
        } else {
            saveVoiceNote()
        }
    ...
    private func saveVoiceNote() {
        ...
        var voiceNote = VoiceNote(
            audioData: audioData,
            duration: audioService.lastRecordingDuration
        )
        voiceNote.transcript = transcriptionManager.transcription?.isEmpty == false ? transcriptionManager.transcription : nil
